...
nightly for both Autobahn directions. Track B: **26 findings, 6 fixed upstream**
and pinned here, all of them whole — H-2's remaining three quarters landed with
[Hermod#32](https://github.com/Vanaheimr/Hermod/pull/32) on 2026-09-24 and H-26
with [Hermod#42](https://github.com/Vanaheimr/Hermod/pull/42) on 2026-09-25.
...
| Hermod, the filter CI gates on (`Tests.HTTP.` + `Tests.HTTPS.`) | ✅ 657, both legs — 537 before H-1 and 562 before H-2's second half. Most of the growth since is Hermod master's own: of the 46 added between 2026-09-24 and 2026-09-25, none came from this repo. **The filter reaches neither `Tests.TCP` nor `Tests.Warden`**, so the ten tests H-26 added run in Hermod's CI and not in ours — worth knowing, given that `AHTTPServer` derives from `ATCPServer` |
| ↳ `Tests.HTTP.` alone | ✅ 656 — the missing one is all of `Tests.HTTPS.` |
